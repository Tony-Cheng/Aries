    ...
    initializeAllListeners() {
        ...
        this.io.on('connection', (socket) => {
            ...
            socket.on('initializeChat', async function (user) {
              ...
              socket.broadcast.emit('newConnectedUser', {userid: user.userid, socketid: socket.id});
            ...
            socket.on('newMessage', async function (msg) {
              ...
              if (msg.userid2 in userIDs) {
                chatIO.to(userIDs[msg.userid]).emit('receiveMessage', {text: msg.text, userid: msg.userid2});
                chatIO.to(userIDs[msg.userid2]).emit('receiveMessage', {text: msg.text, userid: msg.userid});
              } else {
                chatIO.to(userIDs[msg.userid]).emit('receiveMessage', {text: msg.text, userid: msg.userid2});
              }
            });
            ...
            socket.on('NewTwoPersonChat', async function (newChat) {
              ...
              if (typeof newChatID !== "boolean") {
                chatIO.to(userIDs[newChat.user1]).emit('AddedChat', {doesExist: false, userid: newChat.user2, chatid: newChatID, username: newChat.username});
              } else {
